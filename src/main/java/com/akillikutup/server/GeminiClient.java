...
public class GeminiClient {
    ...
    /**
     * Kitap kapağı görselini analiz eder (Vision AI).
     * Kamera ile çekilen kitap kapağından başlık, yazar, ISBN çıkarır.
     *
     * @param base64Image Görselin Base64 kodlanmış hali (data:image/jpeg;base64,... formatında OLMADAN)
     * @param mimeType    Görsel MIME tipi (örn: "image/jpeg", "image/png")
     * @param userApiKey  Kullanıcının API anahtarı (null ise sistem anahtarı kullanılır)
     * @return JSON formatında {baslik, yazar, isbn, kategori} veya hata mesajı
     */
    public static String analyzeBookCover(String base64Image, String mimeType, String userApiKey) {
        ...
        try {
            ...
            // Görsel kısmı: inline base64 data
            ...
            if (result != null && !result.startsWith("API_ERROR") && !result.startsWith("NETWORK_ERROR")) {
                // Gemini yanıtı JSON içerebilir, temizleyelim
    ...
    /**
     * Metin için embedding (vektör) üretir.
     * RAG (Retrieval-Augmented Generation) modülünde kullanılır.
     *
     * @param text       Embedding üretilecek metin
     * @param userApiKey API anahtarı
     * @return float[] embedding vektörü veya null
     */
    public static float[] generateEmbedding(String text, String userApiKey) {
        ...
        try {
            // Gemini embedding API endpoint'i
    ...
    /**
     * Belirtilen modelleri sırayla dener, ilk başarılı yanıtı döndürür.
     */
